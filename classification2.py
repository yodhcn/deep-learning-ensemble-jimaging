...
from balance import ler_BalanceamentoDividido2, num_classes_maper
...
if __name__ == '__main__':
    ...
    # 3 - classifier_3
    # 4 - classifier_4
    classifier_no = 3  # 要训练的目标分类器
    num_classes = num_classes_maper(classifier_no)
    num_epocas = 30
    num_batch_size = 32
    X_treinamento, y_treinamento, X_validacao, y_validacao, X_teste, y_teste = ler_BalanceamentoDividido2(classifier_no)
    ...
    cm = confusion_matrix(np.argmax(y_teste, axis=1), np.argmax(y_pred, axis=1))
    print(cm)
